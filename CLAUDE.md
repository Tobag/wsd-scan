...
install.sh                       — Reproducible installer (auto-discovers scanner, auto-detects IP; system or --user mode; writes .install-record)
...
```bash
# Install (system-wide, default) — auto-discovers scanner, auto-detects local IP
./install.sh
# Install (user-level service, no sudo)
./install.sh --user
# Install with explicit target/self IP (skip discovery)
./install.sh -t http://192.168.0.149:8018/wsd -s 192.168.0.110
# Install with --no-discover to skip auto-discovery and prompt manually
./install.sh --no-discover
...
`install.sh` fills the `wsd-scan.service` template (placeholders like `__TARGET__`, `__SELF_IP__`, `__USER__`) with runtime values. If `-t` is not provided, it auto-discovers WSD scanners via UDP multicast (filtering for `/wsd` endpoints). If `-s` is not provided, it auto-detects the local IP from the default route. It writes `.install-record` so `uninstall.sh` can undo exactly what was installed. Lines prefixed `#@SYSTEM` in the template are system-mode only (stripped in system mode, deleted in `--user` mode). Default is system-wide install (`/etc/systemd/system/`); `--user` installs to `~/.config/systemd/user/`.
