...
The `-t` flag is the WSD endpoint URL of the scanner. The `-s` flag is the local IP the scanner can reach (for callback). Port 6666 is hardcoded for the HTTP event listener.
